...
class ChatFrame(ctk.CTkFrame):
    ...
    def recordButton_callback(self):
        if self.isRecording:
            self.recordButton.configure(text="Start Recording", fg_color="grey")
            self.isRecording = False
            transcribe.stop_record_auto()
        else:
            self.recordButton.configure(text="Stop Recording", fg_color="#fc7b5b")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        container = ChatFrame(master=self, width=600, height=500, bg_color="#f0f0f0")
        container.grid(row=0, column=1, padx=5, pady=20, sticky="nswe")
